...
nacionalidade(X,alemao):-(X=\=1),cigarro(X,prince),not(
          ...
          cor(X,amarela)).
nacionalidade(X,dinamarques):-((X=\=1),(X=\=3)),bebida(X,cha),
...
bebida(X,leite):-X=:=3,not(nacionalidade(X,dinamarques);cor(X,verde);
          cigarro(X,blue_master)).
bebida(X,agua):-(X=\=3),not(nacionalidade(X,dinamarques);cor(X,verde);
          cigarro(X,blue_master)).
bebida(X,cafe):-(X=\=3),cor(X,verde),not(nacionalidade(X,dinamarques);
          nacionalidade(X,ingles);
          cigarro(X,blue_master);cigarro(X,dunhill)).
bebida(X,cerveja):-(X=\=3),cigarro(X,blue_master),
          not(nacionalidade(X,dinamarques);
          cor(X,verde);cor(X,amarela);animal(X,passaros);
          cigarro(X,prince);cigarro(X,dunhill)).
bebida(X,cha):-(X=\=3),nacionalidade(X,dinamarques),
